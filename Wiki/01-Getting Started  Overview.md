...
- **Zones and Cuts** (include or exclude sub-areas inside a region)
- **Spawn Markers** (independent fixed spawn points outside the region flow)
- **Batch spawn and spawn passes** (advanced controls for spawn attempts)
...
### 5) (Optional) Use zones, cuts or markers
```bash
/rsmaddzone <region> <zone_name>
/rsmaddcut <region> <cut_name>
```

- Use zones when only part of the region should spawn mobs.
- Use cuts when you want to exclude an area from the region.
- Use Spawn Markers when you want a simple fixed point independent from regions.

```bash
/rsmmarkercreate <name>
```
...
- Plugin version (currently 1.1.8)
- Hytale server version
- Commands used
- Error messages (if any)
- Server logs (with `/rsmdebug <type> true` enabled if possible)
