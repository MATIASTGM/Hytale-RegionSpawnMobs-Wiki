...
- `RETURN` - Saved as a mode value, but current correction still uses teleport behavior
...
- Uses the spatial region grid to avoid scanning unrelated regions
- Reuses border references instead of rebuilding more work than needed
- Reconciles tracked mobs that are still loaded
...
### Spawn Markers and Leash

Markers have their own system, separate from regional containment.

- Outside combat, a marker mob uses the leash configured on the marker.
- The marker UI leash maximum remains 32 blocks.
- In combat (`LockedTarget`), the marker uses a fixed 48 block distance before teleporting the mob back.
- Marker teleport also has a 3 second cooldown per mob.
- Marker mobs that are tamed or retyped release the marker tracker and stop being controlled by the marker.

This prevents tamed mobs from continuing to return to the marker or being removed when the original time/weather condition stops matching.
