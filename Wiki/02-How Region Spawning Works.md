...
Additional controls can refine this behavior without replacing the core model:

- `spawnPasses` can run more than one spawn attempt in the same cycle.
- `batchSpawnPlacement` can keep a spawn batch closer together when enabled.
- `allowChunkDensityOverflow` allows a batch to finish even when the selected chunk reaches density during that batch.
- Zones and Cuts can include or exclude sub-areas inside a region.
- Spawn Markers are fixed independent points handled outside the region flow.
...
- Increase `spawnPasses` in the region menu if you want more attempts per cycle
...
**Always local fields:** `boundary`, `spawnPoints`, `isActive`, `regionName`, `folderPath`, `maxPerChunk`, `spawnPasses`

> `spawnMode`, `ignoreChunkDensity` and `spawnPasses` are stored per region in current configs, so check the child region when behavior looks different from its parent.
...
Zones and Cuts are optional area controls inside a region.

- **Zones** define where mobs are allowed to spawn inside the region.
- **Cuts** remove areas from spawning, even if they are inside the region boundary.

They do not replace the region boundary. The region still controls activation, capacity and containment. Zones and Cuts only refine valid spawn positions.

Use them when the main region is useful for management, but only part of it should actually receive spawns.

---

## Spawn Markers

Spawn Markers are fixed points independent from regions. They use `markers.json`, `MarkerTrackerComponent` and the `Default_Markers` preset by default.

Important behavior:
- a marker spawns directly at the marker position;
- activation distance controls when a player activates the marker;
- outside combat, the marker uses its configured leash radius;
- in combat, marker leash uses a fixed 48 block distance before teleporting back;
- tamed or retyped marker mobs release the marker tracker and stop counting, teleporting or being removed by marker time/weather conditions.

Use markers when you want a simple direct point. Use regions when you need area capacity, chunk density, zones, cuts, SpawnPoints or regional containment.
...
- `maxAlive` per entity
- `spawnPasses` per region
- zones and cuts for spawn area refinement
- Spawn Markers for independent fixed points
